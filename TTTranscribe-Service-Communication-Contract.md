...
### Nov 30 2025 Updates from Business Engine

- **Job identity**: Business Engine maps its `requestId` to your `jobId`; keep sending `requestId` back in webhooks. If you see retries, respond idempotently using `idempotencyKey`.
- **Holds and monetization**: The Business Engine now places and finalizes credit holds per job. If you emit multiple webhooks for the same `requestId`, always include the final `status` and `usage` so the Engine can reconcile charges once and refund differences.
- **Status cadence**: The Engine caches `/ttt/status/:id` responses for 15s. If you return multiple rapid updates, the latest one within 15s is served.
- **Authentication failures**: If you receive 401/403 from the Engine on webhook delivery, retry with backoff; the Engine auto-heals missing request mappings, so late webhooks still settle the hold.
- **Expected headers**: Continue signing callbacks with `BUSINESS_ENGINE_WEBHOOK_SECRET` (HMAC SHA-256 over `{jobId,requestId,status,usage,timestamp,idempotencyKey}` JSON). The shared secret is already configured in production.
