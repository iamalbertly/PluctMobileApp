        ...
        liveProc.stdout.on('data', (buf) => {
            ...
            for (const line of lines) {
                ...
                if (!re || re.test(line)) {
                    ...
                    logInfo(line, 'Logcat');
                    try {
                        if (/HTTP REQUEST/i.test(line)) logInfo(line, 'HTTP');
                        else if (/HTTP RESPONSE/i.test(line)) logInfo(line, 'HTTP');
                        else if (/(REQUEST_SUBMITTED|Authorization: Bearer|ttt\/transcribe|proxy)/i.test(line)) logInfo(line, 'HTTP');
                    } catch {}
                }
            }
        });
