...
function testShareIntent(url) {
    ...
    if (!post.overall) { reportStepFailure('ShareIntent:post', `Expect ${Steps.ShareIntent.post.join(', ')}`, 'Missing components', { sample: post.xmlSample }); sleep(1500); post = UI.validateComponents('ShareIntent:post', Steps.ShareIntent.post); if (!post.overall) { reportCriticalError('UI Validation Failed', `ShareIntent post components missing: ${JSON.stringify(post.details)}`, 'Core'); return false; } }
    return true;
}
...
function testEnhancementsJourney(url) { try { const aiOk = logcatContains('(AI|metadata|transcript)'); if (!aiOk) { logWarn('AI features not detected', 'Journey'); } const cacheOk = logcatContains('cache'); if (!cacheOk) { logWarn('Smart caching not detected', 'Journey'); } logSuccess('Enhancements journey test passed', 'Journey'); return true; } catch (e) { reportCriticalError('Enhancements Journey Test Exception', e.message || String(e), 'Enhancements'); return false; } }
